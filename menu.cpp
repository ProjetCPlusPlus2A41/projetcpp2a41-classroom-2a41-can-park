...
Menu::Menu(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Menu),    pi(0),
    mainWindow(nullptr) // Initialisation à nullptr
    // Initialize 'pi' with a default value

{
    ui->setupUi(this);
    connect(ui->out, &QPushButton::clicked, this, &Menu::on_button_return_to_main_clicked);
...
//livraisonnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
   ...
   connect(ui->supprimerl, &QPushButton::clicked, this, &Menu::on_pushbutton_supprimer_clikedl);
    ...
    connect(ui->calendar, &QCalendarWidget::clicked, this, &Menu::on_calendarWidget_clicked);

    displayDeliveriesInCalendar();
    ui->datel->setDate(QDate::currentDate());

    connect(ui->mapsl, &QPushButton::clicked, this, &Menu::on_pushButton_showMap_clicked);

    connect(ui->park, &QPushButton::clicked, this, &Menu::openechourouDialog);
    ui->idl->setValidator(new QIntValidator(0, 99, this));

    //prduittttttttttttttttttttttttttttttttttttttttttttttttttttttttttt
    connect(ui->qrt, &QPushButton::clicked, this, &Menu::openQRDialog);
    ui->recherchee->setPlaceholderText("recherche");
   ...
   connect(ui->supprimert, &QPushButton::clicked, this, &Menu::on_pushbutton_supprimer_clikedt);
    ...
    connect(ui->catt, &QPushButton::clicked, this, &Menu::opencatDialog);
    ...
    //employeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
       ...
       connect(ui->supprimere, &QPushButton::clicked, this, &Menu::on_pushbutton_supprimer_clikede);
        ...
        ui->tableViewe->setModel(ESe.afficher());
        connect(ui->face, &QPushButton::clicked, this, &Menu::on_btnPrendrePhoto_clicked);
        connect(ui->presence, &QPushButton::clicked, this, &Menu::openpresenceDialog);
        ui->tableViewe->setModel(ESe.afficher());
    ...
    ui->em->setDate(QDate::currentDate());
    ...
    connect(ui->exm, &QPushButton::clicked, this, [this]() {
        exporterEnExcel(ui->tableViewm->model());
    });
...
ui->dateEdit->setDate(QDate::currentDate());
 ...
 connect(ui->emailc, &QPushButton::clicked, this, &Menu::openEmailDialog);
    ...
    connect(ui->r1, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget->setCurrentIndex(1);
    });
    ...
    connect(ui->r4, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget->setCurrentIndex(2);
    });
    connect(ui->r2, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget->setCurrentIndex(3);
    });
    connect(ui->employee, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget->setCurrentIndex(3);
    });
    connect(ui->re2, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget_4->setCurrentIndex(1);
    });
    connect(ui->re1, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget_4->setCurrentIndex(2);
    });

    connect(ui->re5, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget_4->setCurrentIndex(0);
    });

    connect(ui->av, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget_4->setCurrentIndex(2);
    });
    connect(ui->sale, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget->setCurrentIndex(4);
    });
    connect(ui->r3, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget->setCurrentIndex(4);
    });
...
if(S==0){
        on_stackWidget_currentChanged(5);
        S=1;
}

    });
    connect(ui->r6, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget->setCurrentIndex(5);
if(S==0){
        on_stackWidget_currentChanged(5);
        S=1;
    ...
    connect(ui->r5, &QPushButton::clicked, this, [this]() {
        ui->stackedWidget->setCurrentIndex(6); // Show equipment page
    });
}
...
void Menu::openQRDialog(){
    qrcode *dialog = new qrcode(this);
       dialog->exec();


}
void Menu::on_Chatbot_clicked()
{
    chatbot *dialog = new chatbot(this);
    dialog->exec();

}
void Menu::opencatDialog(){
    catalogue *dialog = new catalogue(this);
       dialog->exec();


}
void Menu::openpresenceDialog(){
    QScopedPointer<rif> dialog(new rif(this));
...
void Menu::openechourouDialog(){
    QScopedPointer<key> dialog(new key(this));
...
void Menu::on_btnPrendrePhoto_clicked()
{
    QString photoID = ui->lineEdit_IDSe->text();  // Récupérer l'ID depuis QLineEdit
        if (photoID.isEmpty()) {
        ...
        query.bindValue(":ide", photoID);  // Lier l'ID à la requête

        if (!query.exec()) {
            QMessageBox::critical(this, "Erreur", "Échec de la requête de recherche de l'ID dans la base de données.");
            return;
        }

        // Vérifier le résultat de la requête
        query.next();
        int count = query.value(0).toInt();  // Récupérer le nombre d'occurrences de l'ID
        if (count == 0) {
            QMessageBox::warning(this, "Erreur", "Cet ID n'existe pas dans la base de données. La photo ne sera pas capturée.");
            return;  // Ne pas prendre de photo si l'ID n'existe pas
        }
    // Chemin du script Python
    QString scriptPath = "C:/firqs/camera_capture.py";

    // Exécuter le script Python avec QProcess
    QProcess process;
    QStringList arguments;
    arguments << scriptPath << photoID;  // Passer l'ID en argument au script
    process.start("python", arguments);

    if (!process.waitForFinished()) {
        QMessageBox::critical(this, "Erreur", "Échec de l'exécution du script Python.");
    } else {
        QMessageBox::information(this, "Succès", "Photo capturée et enregistrée avec succès.");
    }
}
...
void Menu::on_stackWidget_currentChanged(int index)
{
    if (index == 5) {  // Assuming the "matiere" tab is at index 1
        matiere mat; // Create an instance of the matiere class
        if (!mat.verifierExpiration()) {
            QMessageBox::information(this, "Aucune Expiration", "Aucun produit n'est proche de l'expiration.");
        }
    }
}
...
void Menu::displayDeliveriesInCalendar() {
    QSqlQuery query;
    query.prepare("SELECT id_livraison, TO_CHAR(date_livraison, 'YYYY-MM-DD') AS formatted_date FROM LIVRAISON");
    query.exec();

    QTextCharFormat defaultFormat;
    ui->calendar->setDateTextFormat(QDate::currentDate(), defaultFormat);

    while (query.next()) {
        QString dateLivraison = query.value("formatted_date").toString(); // Format 'YYYY-MM-DD'
        QDate deliveryDate = QDate::fromString(dateLivraison, "yyyy-MM-dd");

        if (deliveryDate.isValid()) {
            QString deliveryId = query.value("id_livraison").toString();
            QTextCharFormat format;
            format.setForeground(Qt::blue);
            ui->calendar->setDateTextFormat(deliveryDate, format);
        }
    }
}
...
void Menu::on_calendarWidget_clicked(const QDate &date) {
    ui->listWidgetDeliveries->clear();

    QSqlQuery query;
    query.prepare("SELECT ID_LIVRAISON FROM LIVRAISON WHERE DATE_LIVRAISON = TO_DATE(:date, 'YYYY-MM-DD')");
    ...
    if (query.exec()) {
        ...
        while (query.next()) {
            ...
            ui->listWidgetDeliveries->addItem("Livraison ID: " + QString::number(id));
            found = true;
        }

        if (!found) {
            ui->listWidgetDeliveries->addItem("Aucune livraison pour cette date");
        }
    } else {
        qDebug() << "Erreur lors de la récupération des livraisons:" << query.lastError().text();
    }
}

// Method to load delivery dates and mark them
void Menu::afficherLivraisonsDansCalendrier() {
    QSqlQuery query;
    query.prepare("SELECT DATE_LIVRAISON FROM LIVRAISON");
    ...
    QList<QDate> deliveryDates;
    while (query.next()) {
        QString dateLivraison = query.value("DATE_LIVRAISON").toString();
        QDate deliveryDate = QDate::fromString(dateLivraison, "yyyy-MM-dd");
    ...
    // Create a stylesheet string to highlight the delivery dates
    QStringList deliveryDateStyles;
    for (const QDate &date : deliveryDates) {
        QString dateStr = date.toString("yyyy-MM-dd");
        deliveryDateStyles.append(
            QString("QCalendarWidget QAbstractItemView:enabled QDateTimeEdit[data='%1'] { background-color: red; color: white; }").arg(dateStr)
        );
    }

    // Apply the generated styles for delivery dates
    QString style = R"(
    ...
    // Combine the styles for normal calendar and the delivery dates
    if (!deliveryDateStyles.isEmpty()) {
        style += deliveryDateStyles.join(" ");
    }

    // Apply the final stylesheet
    ui->calendar->setStyleSheet(style);
}
void Menu::on_pushButton_showMap_clicked() {
    // Vérifier si une ligne est sélectionnée
    QItemSelectionModel *selectionModel = ui->tableViewl->selectionModel();

    if (!selectionModel || !selectionModel->hasSelection()) {
        QMessageBox::warning(this, "Avertissement", "Veuillez sélectionner une livraison pour afficher son itinéraire.");
        return;
    }
    ...
    if (!selectedIndex.isValid()) {
        QMessageBox::warning(this, "Avertissement", "Aucune livraison valide sélectionnée.");
        return;
    }

    // Récupérer l'adresse de livraison (supposons qu'elle est dans la colonne 1)
    int row = selectedIndex.row();
    QString adresseLivraison = ui->tableViewl->model()->index(row, 2).data().toString(); // Colonne 1 = Adresse

    if (adresseLivraison.isEmpty()) {
        QMessageBox::warning(this, "Avertissement", "L'adresse de la livraison est vide.");
        return;
    }

    // Définir l'adresse de départ par défaut
    QString depart = "Charguia, Tunis";

    // Construire l'URL pour Google Maps Directions
    QString url = "https://www.google.com/maps/dir/?api=1&origin=" + depart + "&destination=" + adresseLivraison;

    // Ouvrir dans le navigateur par défaut
    bool opened = QDesktopServices::openUrl(QUrl(url));

    if (!opened) {
        QMessageBox::critical(this, "Erreur", "Impossible d'ouvrir la carte. Vérifiez votre connexion Internet.");
    }
}
void Menu::on_button_return_to_main_clicked() {
    if (mainWindow) { // Vérification avant l'utilisation
           mainWindow->show();
       } else {
           qDebug() << "mainWindow is null! Cannot return to MainWindow.";
       }
       this->close();           // Fermer la fenêtre Menu
}

void Menu::setMainWindow(MainWindow *mainWindow) {
